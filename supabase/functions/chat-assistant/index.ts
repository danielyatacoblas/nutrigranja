...
serve(async (req) => {
  ...
  try {
    ...
    // Use service role key to query assistant_api_key (global)
    ...
    if (configError && configError.code !== "PGRST116") {
      ...
      return new Response(
        JSON.stringify({
          ...
          response:
            "Error al acceder a la configuración. Por favor, intenta nuevamente.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!userConfig || !userConfig.api_key) {
      console.log("No API key configured globally");
      return new Response(
        JSON.stringify({
          error: "API key no configurada",
          response:
            "Para usar el asistente, necesitas configurar la API key global de OpenAI. Ve a la configuración del asistente y agrega tu API key.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log("Global config found, API key present:", !!userConfig.api_key);
...
Si te preguntan algo fuera de tu alcance, responde amablemente que solo puedes ayudar con temas relacionados al sistema NutriGranja y búsqueda de proveedores.`;
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature:
          assistantConfig.tone === "formal"
            ? 0.3
            : assistantConfig.tone === "friendly"
            ? 0.7
            : 0.5,
      }),
    });
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return new Response(
          JSON.stringify({
            ...
            response:
              "Tu API key de OpenAI parece ser inválida. Por favor, verifica que sea correcta en la configuración del asistente.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      if (response.status === 429) {
        return new Response(
          JSON.stringify({
            ...
            response:
              "Has excedido el límite de solicitudes de tu API key. Por favor, intenta nuevamente en unos momentos.",
          }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      return new Response(
        JSON.stringify({
          ...
          response:
            "Error al comunicarse con OpenAI. Por favor, verifica tu API key e intenta nuevamente.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (!data.choices || !data.choices[0] || !data.choices[0].message) {
      ...
      return new Response(
        JSON.stringify({
          ...
          response:
            "Respuesta inválida de OpenAI. Por favor, intenta nuevamente.",
        }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    console.log(
      "OpenAI response received successfully, tokens used:",
      tokensUsed
    );
    ...
    return new Response(
      JSON.stringify({
        ...
        response:
          "Lo siento, ocurrió un error interno del servidor. Por favor, intenta nuevamente.",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
